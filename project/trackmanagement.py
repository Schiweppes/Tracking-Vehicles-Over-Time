...
class Track:
    ...
    def __init__(self, meas, id):
        ...
        p_sens = np.ones((4, 1))
        p_sens[:3] = meas.z[:3]
        p_veh = meas.sensor.sens_to_veh @ p_sens
        self.x = np.zeros((6,1))
        self.x[:3] = p_veh[:3]
        ...
        #self.score = 0
        pos_p = M_rot @ meas.R @ np.transpose(M_rot)
        vel_p = np.matrix([[params.sigma_p44**2, 0, 0],
                           [0, params.sigma_p55**2, 0],
                           [0, 0, params.sigma_p66**2]])

        self.P = np.zeros((6, 6))
        self.P[:3,:3] = pos_p
        self.P[3:6, 3:6] = vel_p
    ...
    def manage_tracks(self, unassigned_tracks, unassigned_meas, meas_list):  
        ...
        for i in unassigned_tracks:
            ...
            if meas_list: # if not empty
                if meas_list[0].sensor.in_fov(track.x):
                    ...
                    if track.score > params.delete_threshold:  # set up limitation
        ...
        for track in self.track_list:
            if track.score <= params.delete_threshold:
                ...
                track.score < 0.1): # check covariances
